    ...
    <section class="content">
        <div class="container-fluid" ">
            ...
            <div class="marcacion-col-left">
                <img src="<?= htmlspecialchars($miembro['url_foto'] ?? '/sysgym/public/images/avatar.png') ?>"
                    alt="Foto" class="foto-miembro" id="foto-miembro">
                ...
                <ul class="info-miembro-list">
                    <li><b>ID:</b> <span id="id-miembro"><?= htmlspecialchars($miembro['id_miembro'] ?? '-') ?></span>
                    </li>
                    <li><b>Tipo Documento:</b> <span
                            id="tipo-documento-miembro"><?= htmlspecialchars($miembro['tipo_documento'] ?? '-') ?></span>
                    </li>
                    <li><b>Número Documento:</b> <span
                            id="numero-documento-miembro"><?= htmlspecialchars($miembro['numero_documento'] ?? '-') ?></span>
                    </li>
                    <li><b>Correo:</b> <span
                            id="correo-miembro"><?= htmlspecialchars($miembro['correo_electronico'] ?? '-') ?></span>
                    </li>
                    <li><b>Teléfono:</b> <span
                            id="telefono-miembro"><?= htmlspecialchars($miembro['telefono'] ?? '-') ?></span></li>
                    <li><b>Dirección:</b> <span
                            id="direccion-miembro"><?= htmlspecialchars($miembro['direccion'] ?? '-') ?></span></li>
                    <li><b>Fecha Nacimiento:</b> <span
                            id="fecha-nacimiento"><?= htmlspecialchars($miembro['fecha_nacimiento'] ?? '-') ?></span>
                    </li>
                    <li><b>Género:</b> <span
                            id="genero-miembro"><?= htmlspecialchars($miembro['genero'] ?? '-') ?></span></li>
                    <li><b>Contacto Emergencia:</b> <span
                            id="contacto-emergencia"><?= htmlspecialchars($miembro['contacto_emergencia_nombre'] ?? '-') ?>
                            (<?= htmlspecialchars($miembro['contacto_emergencia_telefono'] ?? '-') ?>)</span></li>
                </ul>
            ...
            <div class="marcacion-col-main">
                <div class="marcacion-membresia">
                    <span><b>Fecha de Inicio:</b> <span
                            id="fecha-inicio"><?= htmlspecialchars($membresia['fecha_inicio']) ?></span></span>
                    <span><b>Fecha de Vencimiento:</b> <span
                            id="fecha-fin"><?= htmlspecialchars($membresia['fecha_fin']) ?></span></span>
                    <span><b>Días Vigentes:</b> <span
                            id="dias-vigentes"><?= htmlspecialchars($membresia['dias_vigentes']) ?></span></span>
                    <span><b>Días Totales:</b> <span
                            id="dias"><?= htmlspecialchars($membresia['dias_total']) ?></span></span>
        ...
        <div style="margin-top:18px; width:100%;">
            <input type="text" id="manual-numero-documento" class="form-control" placeholder="Número de documento"
...
<div id="modalMembresiaVencida"
    ...
    <div
        style="background:#fff; padding:32px 24px; border-radius:12px; max-width:350px; margin:auto; text-align:center;">
        ...
        <button onclick="cerrarModalMembresiaVencida()"
            style="margin-top:18px; background:#1976d2; color:#fff; border:none; border-radius:8px; padding:8px 24px;">Cerrar</button>
    </div>
</div>
...
async function verificarHuella() {
    try {
        ...
        if (data.success && data.id_miembro) {
            ...
            fetch('http://localhost/SysGym/App/controllers/miembros/get_miembro.php?id=' + encodeURIComponent(data
                    .id_miembro))
                .then(resp => resp.json())
                .then(usuario => {
                    if (usuario.success && usuario.miembro) {
                        // ...actualizas los datos del usuario...
                        const rutaFoto = usuario.miembro.url_foto ?
                            '/SysGym/App/' + usuario.miembro.url_foto.replace(/^\/?/, '') :
                            '/sysgym/public/images/avatar_default.png';
                        document.getElementById('foto-miembro').src = rutaFoto;
                        document.getElementById('nombre-miembro').textContent = usuario.miembro.nombres + ' ' +
                            usuario.miembro.apellidos;
                        ...
                        document.getElementById('tipo-documento-miembro').textContent = usuario.miembro
                            .tipo_documento || '-';
                        document.getElementById('numero-documento-miembro').textContent = usuario.miembro
                            .numero_documento || '-';
                        document.getElementById('correo-miembro').textContent = usuario.miembro
                            .correo_electronico || '';
                        ...
                        document.getElementById('direccion-miembro').textContent = usuario.miembro.direccion ||
                            '';
                        document.getElementById('fecha-nacimiento').textContent = usuario.miembro
                            .fecha_nacimiento || '';
                        ...
                        fetch('http://localhost/SysGym/App/controllers/accesos/registrar_acceso.php', {
                                ...
                                body: 'id_miembro=' + encodeURIComponent(data.id_miembro) +
...
function actualizarHistorialAccesos(idMiembro) {
    fetch('http://localhost/SysGym/App/controllers/accesos/get_historial.php?id_miembro=' + encodeURIComponent(
            idMiembro))
        .then(resp => resp.json())
        .then(data => {
...
document.getElementById('btn-manual-ingreso').addEventListener('click', async () => {
    ...
    const resp = await fetch(
        'http://localhost/SysGym/App/controllers/miembros/get_miembro_doc.php?numero_documento=' +
    ...
    const rutaFoto = usuario.miembro.url_foto ?
        ...
        '/sysgym/public/images/avatar_default.png';
    document.getElementById('foto-miembro').src = rutaFoto;
    document.getElementById('nombre-miembro').textContent = usuario.miembro.nombres + ' ' + usuario.miembro
        .apellidos;
    document.getElementById('estado-miembro').textContent = usuario.miembro.estado || '';
    document.getElementById('id-miembro').textContent = usuario.miembro.id_miembro;
    ...
    fetch('http://localhost/SysGym/App/controllers/accesos/registrar_acceso.php', {
            ...
            body: 'id_miembro=' + encodeURIComponent(usuario.miembro.id_miembro) +
